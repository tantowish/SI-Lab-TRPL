...
<div class="max-w-2xl p-8">
    <form action="{{ route('schedule.update') }}" method="post">
        ...
        @csrf
        <div class="mb-6">
            <label for="tanggal" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Laboratorium</label>
            <div class="relative w-full">
                ...
                <input datepicker-format="dd-mm-yyyy" datepicker datepicker-autohide type="text" class="bg-white border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-main focus:border-main block w-full pl-10 p-2.5  dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-main dark:focus:border-blue-500" placeholder="Pilih tanggal" value="{{ \Carbon\Carbon::parse($schedule->start_time)->format('d-m-Y') }}">
            </div>
        </div>
  
        <div class="mb-6">
            <label for="laboratorium" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Laboratorium</label>
            <select id="laboratorium" name="laboratorium" class="bg-white border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-main focus:border-main block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-main dark:focus:border-main">
                <option value="pick">pilih</option>
                @foreach ($laboratorium as $lab)
                    <option value="{{ $lab->laboratorium_id }}" {{ $lab->laboratorium_id == $schedule->lab_id ? 'selected' : '' }}>
